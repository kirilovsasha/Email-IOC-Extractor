...
### GUI / UX
- Сводка результатов: рядом с вердиктом только счётчик доказательств (без дубля вердикта / breakdown)
- Вкладки: стабильные короткие подписи + `tkraise` вместо `pack_forget` — без прыжков интерфейса в EXE
- Убрана кнопка / хоткей Msg-ID; Message-ID остаётся в тексте «В тикет»

### Сборка
- По умолчанию Full EXE (`build_exe.bat` / CI `build-exe`); Lite — `build_exe.bat --lite`
