...
### Исправления
- Пересланные письма (Gmail/Outlook/RU-баннер), BOM и пустые строки в начале больше не «съедают» `From` и цепочку `Received`: hops считаются, поле From заполняется
- MSG: подтягиваются все `Received` (не только первый) и пустой From из метаданных extract-msg
...
- Архивы не распаковываются (и без пароля): имена и флаги — сигнал, содержимое членов — для песочницы
