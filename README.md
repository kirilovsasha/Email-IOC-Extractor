...
- Тулбар: **Источник** (открыть/папка) → **Буфер** (копировать / тикет) → **Экспорт**
- Копирование: `type|value`, `value`, `csv`, **defanged**, **defanged|type**
- **Тикет** — шаблон handoff в буфер; **Msg-ID** — Message-ID / campaign
- Фильтры: типы IOC; **Скрыть шум** (прокси URL / allowlist / частные IP);
  **Фокус** (только denylist / к разбору)
