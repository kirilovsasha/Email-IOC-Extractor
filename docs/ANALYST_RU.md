...
4. ПКМ по IOC → **В allowlist** или **Сменить вердикт…**.
5. Тикет (Ctrl+H) / JSON / CSV / Batch CSV (Ctrl+E). В GUI других форматов нет.
6. Пакет: таблица файлов; Ctrl+N/P — следующее письмо; Ctrl+Shift+N/P — только suspicious/malicious; peers → diff кампании. На вердикте строка кампании, если From-домены разошлись.
...
9. ПКМ → Feedback FP / FN / подтвердить вердикт; пароль ZIP/7z и переразбор (RAR не расшифровывается).
...
ClickFix (Win+R, powershell -enc, mshta), HTML из картинки и http-ссылки, поддельный Authentication-Results в теле, Excel 4.0 / XLM, Resent-From с другим доменом, Message-ID ≠ From в строке LOW. Календарная митигация не применяется, если в ICS есть URL или ATTACH. Пароль в теле вместе с шифрованным Office считается так же, как пароль к архиву.

## Encrypted archive

ZIP/7z с паролем не извлекается, пока пароль не задан: ПКМ → «Пароль архива и переразбор…» (только эта сессия). RAR с паролем не расшифровывается — остаётся флаг и scrape имён. ПКМ → «Заметка: шифрованный архив» для ITSM. Пароль запрашивайте out-of-band; не открывайте на рабочей станции без песочницы.
