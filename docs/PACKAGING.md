...
Запароленный архив не распаковывается (это сигнал).
Правила YARA в артефакт не копируются. `yara-python` попадает в EXE, только если пакет уже установлен; CI его не ставит.
Скан выключен, пока в настройках не заданы путь и галочка «Сканировать YARA», либо пока не передан `--yara-rules`.
