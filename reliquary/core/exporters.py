...
def export_misp(result: AnalysisResult, path: str | Path, iocs: list[Ioc] | None = None) -> Path:
    ...
    payload = {
        "Event": {
            "uuid": event_uuid,
            "info": f"Reliquary export: {result.source_path}",
            "date": now,
            "threat_level_id": "2",
            "analysis": "1",
            "distribution": "0",
            "Attribute": attributes,
            "Tag": [{"name": "reliquary"}, {"name": "tlp:amber"}],
        }
    }
    out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return out
...
def export_opencti(result: AnalysisResult, path: str | Path, iocs: list[Ioc] | None = None) -> Path:
        ...
        objects.append(
            {
                ...
                "description": ioc.context,
                "pattern_type": "stix",
                "pattern": _stix_pattern(ioc) or f"[x-reliquary:value = '{ioc.value}']",
                "valid_from": now,
                "labels": [ioc.ioc_type.value, *ioc.tags, "reliquary"],
    ...
    payload = {
        ...
        "objects": objects,
        "x_reliquary_source": result.source_path,
    }
    out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return out
...
def _yara_rule_id(name: str) -> str:
    ...
    return cleaned[:120] or "Reliquary_Export"
...
def export_yara(
    ...
    base = _yara_rule_id(f"Reliquary_{Path(result.source_path).stem}_{now.replace('-', '')}")
    ...
    lines: list[str] = [
        "// Generated by Reliquary — offline IOC to YARA export",
    ...
    def _emit_rule(suffix: str, group: list[Ioc], *, use_hash_module: bool) -> None:
        ...
        lines.append("{")
        lines.append("    meta:")
        lines.append(f'        description = "Reliquary IOC export ({suffix})"')
        lines.append('        author = "Reliquary"')
    ...
    if lines[-1] == "" and "rule " not in "\n".join(lines):
        ...
        lines.append("{")
        lines.append("    meta:")
        lines.append('        description = "Reliquary export — no usable IOCs"')
        lines.append('        author = "Reliquary"')
        lines.append(f'        date = "{now}"')
        lines.append("    condition:")
        lines.append("        false")
